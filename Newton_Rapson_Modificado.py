...
def Newton_Rapson_Modificado(funcion,X_i,cifras_significativas,muestra_valores,muestra_tabla,muestra_raiz,muestra_grafica):
    global mostrar
    valores_mostrar = ""
    tabla_resultados = ""
    valor_raiz = ""
    ...
    # Parsear la función de entrada
    funcion = sp.sympify(funcion.replace("e", "E"))
    ...
    try:
        pruebaDeConvergencia = abs((f(X_i) * dd(X_i)) /( (d(X_i) )** 2))
    ...
    if pruebaDeConvergencia < 1:
        ...
        while True:
            try:
                if d(X_i)==0:
                    ...
                    messagebox.showerror("¡ ERROR CRITICO !",message=mensaje_)
                    return
                X_r=X_i-((f(X_i)*d(X_i))/(((d(X_i))**2)-f(X_i)*dd(X_i)))  #se evalua en la formula
                Ea=abs(((X_r-valor_anterior)/X_r)*100) #encontramos el error aproximado
                df.loc[i-1]=[i,X_i,f(X_i),d(X_i),X_r,Ea]       
                if Ea<Es:
                    break
                X_i=X_r
                valor_anterior=X_r
        ...
        mensaje_ = f"El punto {X_i} no converge, ingrese otro valor inicial"
        messagebox.showerror("¡ ERROR CRITICO !",message=mensaje_)
        mostrar = False
...
def Ventana_Newton_Rapson_Modificado(frame,ventana2,ventana):
        ...
        #SI uno o varios estan vacios
            ...
            if numero_valido(valor_inicial)  and numero_valido(cifras_significativas)  :
                if Validacion_Funcion(funcion) == True:
                    ...
                    Newton_Rapson_Modificado(funcion,valor_inicial,cifras_significativas,muestra_valores,muestra_tabla,muestra_raiz,muestra_grafica)
