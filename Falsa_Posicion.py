...
def Falsa_Posicion(funcion,X_i,X_u,cifras_significativas,valorReal,muestra_valores,muestra_tabla,muestra_raiz,muestra_grafica):
    global mostrar
    valores_mostrar = ""
    tabla_resultados = ""
    valor_raiz = ""
    ...
    X_r=0
    # Paso 2: Verifificar si la solucion esta en el intervalor seleccionado 
    ...
    try:
        if f(X_i)*f(X_u) <0: # si puede hacer el codigo de falsa posicion
            ...
            df=pd.DataFrame(columns=["Iteracion","Xi","Xu","f(Xi)","f(Xu)","Xr","f(Xr)","f(Xi)*f(Xr)","Condicion","Ev %","Ea %"])
            Ea_=0
            while True:
                X_r=X_u-(((f(X_u))*((X_i)-(X_u)))/((f(X_i))-(f(X_u)))) # se hace la primera aproximacion con la formula de falsa posicion
                ...
                if f(X_i)*f(X_r)<0: #si la multiplicacion es menor a cero
                    condicion="<0"
                    df.loc[i-1]=[i,X_i,X_u,f(X_i),f(X_u),X_r,f(X_r),f(X_i)*f(X_r),condicion,Ev,Ea_] 
                    X_u=X_r
...
def Ventana_Falsa_Posicion(frame,ventana2,ventana):
        ...
        #SI uno o varios estan vacios
            ...
            if numero_valido(valor_inicial) and numero_valido(valor_final) and numero_valido(cifras_significativas) and numero_valido(valor_real) :
                if valida == True:
                    ...
                    Falsa_Posicion(funcion,valor_inicial,valor_final,cifras_significativas,valor_real,muestra_valores,muestra_tabla,muestra_raiz,muestra_grafica)
