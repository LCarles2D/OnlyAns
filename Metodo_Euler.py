...
def Metodo_Euler(funcion,valor_inicial_x,valor_inicial_y,valor_h,valor_final,hacia_adelante,hacias_atras,centrado,muestra_valores,muestra_tabla,muestra_raiz,muestra_grafica):
    global mostrar
    x = sp.symbols("x")
    y = sp.symbols("y")
    ...
    valores_mostrar = f"f(x,y) = {funcion}\nX0 = {valor_inicial_x}\t\tY0 = {valor_inicial_y}\nh = {valor_h}\nX final = {valor_final}"
    i =1

    df=pd.DataFrame(columns=["n","Xn","Yn"])

    if hacia_adelante == True:
        while valor_inicial_x < valor_final:
            valor_inicial_y = valor_inicial_y + valor_h * f(valor_inicial_x, valor_inicial_y)
            valor_inicial_x = valor_inicial_x + valor_h

            df.loc[i-1] = [i, valor_inicial_x, valor_inicial_y]
            
            i+=1


        respuesta = f"Respuesta //  y({valor_final}) = {valor_inicial_y}"
    ...
    elif hacias_atras == True:
        
        while valor_inicial_x < valor_final:
            y_intermedia = valor_inicial_y + valor_h * f(valor_inicial_x,valor_inicial_y)
            valor_inicial_x = valor_inicial_x + valor_h
            valor_inicial_y = valor_inicial_y + valor_h * f(valor_inicial_x, y_intermedia)
...
def Ventana_Metodo_Euler(frame,ventana2,ventana):
    global marco_muestra_valores
    global canvas
    ...
    etiqueta_ingreso_valor_final.place(x=900,y=20)
    ...
    todas_formulas = ["Hacia atras","Hacia adelante","Centrado"]
    #Cramos el menu
    ...
    ingreso_valor_inicial_x.place(x=250,y=60)
    ...
    ingreso_valor_inicial_y.place(x=450,y=60)
    ...
    ingreso_valor_h.place(x=655,y=60)
    ...
    ingreso_valor_final.place(x=900,y=60)
    ...
    def Validacion():
        ...
        if x0 == "" and y0 == "" and h == "" and valor_final == "" and funcion == "" and seleccion_formula == "":
            messagebox.showerror("¡ ERROR CRITICO !",message="Debe llenar todos los campos")
        #SI uno o varios estan vacios
        elif x0 == "" or y0 == "" or h == "" or valor_final == "" or funcion == "" or seleccion_formula == "":
            messagebox.showerror("¡ ERROR CRITICO !",message="Debe llenar todos los campos")
            ...
            if numero_valido(x0) and numero_valido(y0) and numero_valido(h) and numero_valido(valor_final) :
                if valida == True :
                    ...
                    if seleccion_formula == "Hacia atras":
                        hacias_atras_ = True
                    elif seleccion_formula == "Hacia adelante":
                        hacia_adelante_ = True
                    elif seleccion_formula == "Centrado":
                        centrado_ = True
                    ...
                    Metodo_Euler(funcion,x0,y0,h,valor_final,hacia_adelante_,hacias_atras_,centrado_,muestra_valores,muestra_tabla,muestra_raiz,muestra_grafica)                    
                    ...
                    messagebox.showerror("¡ ERROR CRITICO !",message="Solo debe ingresar valores numericos en los campos que se requieren")
